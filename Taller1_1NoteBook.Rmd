...
En el presente trabajo se presenta un ejercicio práctico tomado del articulo ...., con el fin de comparar varios métodos de suavizamiento de curvas.

Este ejercicio busca simular una función afectada por ruido y suavizarla con diferentes métodos para posteriormente, reconocer el funcionamiento y evaluar el desempeño de cada uno de estos.
...
En el articulo se propone el uso de una función coseno para generar la señal. La idea es generar una función con ruido, la que se quiere suavizar, y una función sin ruido que, para el desarrollo del ejercicio, servirá para medir el desempeño de los métodos de suavizamiento.

Para la implementación en R se define una función f que recibe 5 parámetros como entrada y devuleve a la salida un vector de datos que representan una versión muestreada de la función $f:R\longrightarrow R \textbf{ }| f(t) = Acos(2\pi t+ \phi)+\epsilon$, los parámetros de la función definida en R corresponden con las constantes de la función anterior, es decir, A es la amplitud $A$, phi el ángulo de fase $\phi$, t el vector de tiempo sobre el que computa la verisón muestrada de f, y e es el error $\epsilon$ que repsentará el ruido. Adicionalmente se agrega un parámetro binario state que se coloca en 1 si se desea agregar el ruido y 0 si se desea obtener la función real sin el error.

Para determinar una medida del desempeño de los métodos de suavizamiento (o regresión) se utiliza el error cuadrático medio con siglas en el ingles MSE.
...
donde n es el tamaño del vector de tiempo t y $f_d$, $\hat{f_d}$ son las versiones muestradas de $f$ y $\hat{f}$ respectivamente.
...
Aparte del MSE se requieren hacer varias simulaciones del funcionamiento de cada método de suavizamiento para poder decidir sobre el desempeño de estas. Para esto, el articulo propone crear 100 funciones distintas, es decir, con constantes de amplitud y fase diferentes para suavizar cada una de estas. Lo anterior permite tener 100 MSE para cada uno de los métodos y posteriomente realizar un boxplot que permitar compararlos.

El error que se suma a la función correponde a una distribución normal con varianza igual a 0.07. En esta implemnetación se crea una matriz 50x100 donde cada columna posee la función de error que se le suma a cada una de las 100 funciones.

La amplitud y la fase de las 100 funciones se seleccionan aleatoriamente, en particular se utiliza la distribución de rayleigh con parámetro $\sigma$ igual a 0.3 para la amplitud, según lo que se porpone en el articulo.
...
En esta parte se crean las variables que almacenaran la información de las versiones suavizadas de cada una de las 100 funciones.
...
Acontinuación se muestran gráficos que comparan los resultados para la regresión, el suavizamiento CV y GCV y suavizamiento P-spline. Para cada uno de ellos se muestran 5 curvas, la curva sin ruido o real, la curva con ruido o simulada y las curvas suavizadas para 5, 15 y 25 knots.
