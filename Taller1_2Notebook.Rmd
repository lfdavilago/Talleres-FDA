...
Se tienen 7 procesos aleatorios relacionados con la respuesta espectrográfica de mestras de azucar a determinandas longitudes de onda de exitación.

El conjusto de datos representa la información medida durante la espectrografía para 7 longitudes de onda de exitación. Por cada una de estas se registraron las señales del espectro muestreadas en intervalos de 0.5 nm y se tomaron 268 muestras en momentos diferentes. Es decir que cada proceso consta de 268 realizaciones y cada una de estas muestredas con intervalos de 0.5 nm.

Para el desarrollo de este ejercicio práctico se escoge uno de los procesos y se determinan los estadisticos descriptivos para este. Cabe resaltar que sería posible realizar la estadistica descriptiva sobre todos los 7 procesos al tiempo utilizando las versiones multivariadas de profundidad, pero para los alcances de este taller solo se utilizará la respuesta a una de las 7 longitudes de onda.

Se revisa gráficamente la respuesta a cada longitud de onda de exitación para escoger un proceso con "buenas" caráteristicas de suavidad para que el suavizamiento con B-splines funcione adecuadamente. Con respecto a la forman se pueden crear dos grupos de respuestas, el primer grupo se relaciona con respuestas que "prefieren" un solo valor de longitud de onda y gráficamente se puede ver como la amplitud del espectro para algún valor es mucho mayor que el resto de la respuesta, su forma es similar a un $sinc(t)$. El segundo grupo son respuestas donde la amplitud del espectro entre longitudes diferente no cambia de forma significativ, o bien son sería posible decir a simple vista que hay prefrencia sobre alguna longitud de onda en la respuesta.
...
```{r}
sugar <- readMat("~/Documents/FDA/R/Taller 1/sugar_Process/data.mat")
```
...
Se selecciona el proceso que corresponde a la longitud de exitación de 305 nm, cambiando el valor de ex_wl se pueden determinar los estadisticos descriptivos para otros procesos, sin embargo, se deben revisar a detalle los resultados verificando que el suavizamiento de las curvas sea aceptable.
...
Para que el suavizamiento de todo el proceso se realize bajo el mismo parámetro $\lambda$ se define la función minimo.lambda() que sirve para determinar el valor de lamda que minimiza el promedio del criterio de validación cruzada genralizada entre todas las curvas de la muestra.
...
Gráficamente se realiza una comparación de los ajustes para decidir sobre que tan suave es el ajuste y además determinar si este no agrega más suavidad de la real a las curvas. Esta selección se hace bajo criterio personal y permite descartar bases con un número elevado de knots (mayor a 25 knots), pues si bien el error mínimo cuadratico y el GCV pueden parecer más optimos, las gráficas muestran que las curvas suavizadas conservan gran proporción del ruido de las señales originales. Por otra parte se encuentran bases con un número bajo de knots (menos 15 knots) que no solo eliminan el ruido de la señal sino que tambien le quitan su "forma" original. Las siguientes gráficas se utilizan para comparar la forma de las curvas originales con ruido y las curvas suavizadas para 15, 20 y 25 knots.
...
A partir de los gráficos anteriores se decide que, para bases entre 15 y 25 knots, no hay cambios significativos en la "forma" de las curvas ajustadas con relación a la curvas originales. Finalmente se utiliza el criterio GCV y el MSE para verificar cual de estas 3 bases (que conservan la forma y eliminan el ruido) se desempeña mejor. Para esto se realizan boxplot que permiten decidir sobre cual de las bases es mejor en esta aplicación.
...
Los boxplot anteriores permiten notar, que a pesar de no haber un cambio seignificativo en la forma de las curvas, si existe un cambio notable entre la base con 25 knots y las otras 2 en cuanto al MSE y el GCV. Por esta razón se decide utilizar la base B-spline con 25 knots y un valor de $\lambda$ igual a 154.8.
...
Con las funciones suavizadas se obtienen 2 outlier con el método de profundidad por bandas y a estas se le agregan 4 outlier más obtenidos con el mismo método pero a partir de las derivadas de primer orden. Acontinuación se muestra el index en el conjunto de datos de la curva mediana y de los outliers.
...
En el siguiente gráficos se muestra la curva mediana en color negro, los outlier obtenidos sobre las curvas suavizadas en color rojo, y los outlier obtenidos bajo la primera derivada en color amarillo.
...
El boxplot de las cruvas suavizadas permite encontrar outliers relacionados con la amplitud de las cruvas y la profundidad entre estas, por lo tanto considera como outlier las dos curvas con mayor amplitud y que visualmente se pueden distinguir del conjunto de funciones por aparecer en la mayoría del intervalo sin curvas cercanas.

Los outliers detectados en el boxplot funcional de las derivadas de primer orden, ya no consideran como datos atipicos a funciones que se encuentren separadas del conjunto, sino a quellas funciones que presentan "formas" diferentes en determinado intervalo. Este tipo de curvas outliers son más dificiles de identifcar "a ojo" y se debe tener espcial cuidado con estas pues dependiendo de la naturaleza del proceso, se debe verificar si existen o no razones que justifiquen su aparaición en la muestra para no descartarlas. Para este ejercio solo se observa que los outlier detectados en las derivadas de primer orden tienen en común que para una región alrededor de 500 presentan oscilaciones pronunciadas que aparentemente no están presentes en la mayoría de la muestra y se verifica en el boxplot pues los outlier aparecen es este mismo intervalo.
...
Para esta parte se utilizan funciones que reciben dtaos de tipo funcional fdata por esto se crea la muestra para el proceso sin los outliers pero como datos funcionales utilizando la función fdata().

Los estadisticos acontinuación se determinaron con funciones de la librería ...., estas se definieron directamente en la implemntación para no cargar la librería completa y son bootstrap que generan varias simulaciones para determininar los estimadores con su región de confianza. El reporte de progreso de los bootstrap se ha comentado para evitar que se visualice en el documento.
...
plot(media.boostrap.bajo$statistic
     , lty = 1, col = "darkorange", ylim = c(min(media.boostrap.bajo$resample)
                                             ,max(media.boostrap.bajo$resample)), main = "Funciones Media ", xlab = "Longitud de Onda (t)"
     , ylab = expression(hat(x)(t)), cex.lab = 0.8, cex.main = 1)

legend("topleft", legend = c("F. Media", "F. Media - Alto", "Región de Confianza - Bajo"
                             ,"Región de Confianza - Alto"), cex = 0.75, lwd = 2
       , col = c("chartreuse4","gold","cyan3", "darkorange"), inset = c(0.01,0.05), bty = "n")
```
...
drape.plot(long.onda,long.onda,eval.var.baja, expand = 0.5, col = heat.colors(20)
           , main = "Función Covarianza", horizontal = F, ticktype = "detailed"
           , xlab = "t" , ylab = "t", zlab = "Covarianza")
